...
class Window(Style):
    def __init__(self, theme='simplex'):
        ...
        self.theme_settings(
            theme, {"TNotebook.Tab": {"configure": {"padding": [40, 10]}}})
        ...
        self.billing_container = BillingFrame(
            self.billing_frm, name='billing_container')
        self.database_container = DatabaseFrame(
            self.database_frm, name='database_container')
...
class BillingFrame(Frame):
    ...
    def define_widget(self):
        ...
        self.add_remove = [
            ('product_sno', 'SNO', 1, (self.master.register(lambda P: self.find_product_by_sno(
                P, self.add_remove_container)), '%P'), NORMAL, 'xterm'),
            ('product_code', 'Product Code', 1, (self.master.register(
                lambda P: self.find_product_by_code(P, self.add_remove_container)), '%P'), NORMAL, 'xterm'),
            ('product_name', 'Product Name', 1, lambda: True, DISABLED, 'arrow'),
            ('quantity', 'Qty', 1, (self.master.register(self.update_amount), '%P'),
             NORMAL, 'xterm', lambda: self.update_amount(self.quantity_input.get())),
        ...
        self.billing_details_frm = ttk.Labelframe(
            self.left_frm, text=' Customer Details ', name='billing_details_frm')
        self.billing_details_container = Frame(self.billing_details_frm)
        self.billing_details_input_container = [Frame(
            self.billing_details_container, name=i+'_container') for i, *_ in self.customer_details]

        self.customer_details_lb = {
            i+'_lb': ttk.Label(self.billing_details_input_container[c], text=j, name=i+'_lb') for c, *v in enumerate(self.customer_details) for i, j, k in v}
        self.customer_details_input = {i+'_input': ttk.Entry(
            self.billing_details_input_container[c], text=j, name=i+'_input', width=k) for c, *v in enumerate(self.customer_details) for i, j, k in v}
        self.__dict__.update(
            {**self.customer_details_lb, **self.customer_details_input})

        self.product_lst_frm = ttk.Labelframe(
            self.left_frm, text=' Available Products ', name='product_lst_frm')
        self.product_lst_container = Frame(
            self.product_lst_frm, name='product_lst_container')
        self.product_lst = ttk.Treeview(
            self.product_lst_container, selectmode='browse', name='product_lst')
        self.product_lst_scrollbar = ttk.Scrollbar(
            self.product_lst_container, orient="vertical", command=self.product_lst.yview)

        self.add_remove_frm = Frame(self.left_frm, name='add_remove_frm')
        self.add_remove_container = Frame(
            self.add_remove_frm, name='add_remove_container')
        self.add_remove_widget_container = [Frame(
            self.add_remove_container, name=i+'_container') for i, *_ in self.add_remove+['add_btn']]

        self.add_btn_top_dummy = ttk.Label(
            self.add_remove_widget_container[-1], text=' ')
        self.add_btn = ttk.Button(
            self.add_remove_widget_container[-1], text='Add', width=10, cursor='hand2', command=self.add_item_to_purchase)
        add_remove_lb = {i+'_lb': ttk.Label(self.add_remove_widget_container[c], text=j, name=i+'_lb')
                         for c, *v in enumerate(self.add_remove) for i, j, *_ in v}
        add_remove_input = {i+'_input': (ttk.Entry if i != 'quantity' else ttk.Spinbox)(self.add_remove_widget_container[c], **(dict(width=k, name=i+'_input', validate='key', validatecommand=l, state=m, cursor=n, command=o[0] if o else None) | ({
            'from_': 1, 'to': 9999} if i == 'quantity' else {}))) for c, *v in enumerate(self.add_remove) for i, _, k, l, m, n, *o in v}
        self.__dict__.update({**add_remove_lb, **add_remove_input})

        self.purchased_lst_frm = ttk.Labelframe(
            self.left_frm, text=' Purchased Products ')
        self.purchased_lst_container = Frame(self.purchased_lst_frm)
        self.purchased_lst = ttk.Treeview(
            self.purchased_lst_container, selectmode='browse')
        self.purchased_lst_scrollbar = ttk.Scrollbar(
            self.purchased_lst_container, orient="vertical", command=self.purchased_lst.yview)

        self.receipt_view_frm = ttk.Labelframe(
            self.right_frm, text=' Receipt Preview ')
        ...
        self.total_input = ttk.Entry(
            self.total_frm, font=(None, 20, 'bold'), width=0)

        self.tax_frm = Frame(self.summarize_top_frm)
        self.tax_lb = ttk.Label(self.tax_frm, text='Tax')
        self.tax_input = ttk.Entry(
            self.tax_frm, font=(None, 20, 'bold'), width=0)

        self.grand_total_lb = ttk.Label(self.summarize_frm, text='Grand Total')
        self.grand_total_input = ttk.Entry(
            self.summarize_frm, font=(None, 30, 'bold'), justify=CENTER)
        self.checkout_btn = ttk.Button(
            self.summarize_frm, style='checkout.primary.TButton', text='Check Out', cursor='hand2')

        self.functions_frm = ttk.Labelframe(self, text=' Functions ')
        self.functions_container = Frame(self.functions_frm)
        self.function_btns = [ttk.Button(self.functions_container, text=i, name=i.replace(
            ' ', '_').lower(), style=j, cursor='hand2', command=c, padding=(0, 0)) for i, j, c in functions]

    def place_widget(self):
        [i.pack(side=LEFT, expand=True, fill=X, padx=10)
         for i in self.billing_details_input_container]
        [self.billing_details_container.nametowidget('{0}_container.{0}_lb'.format(i)).pack(
            side=TOP, expand=True, fill=X, pady=(0, 5)) for i, j, k in self.customer_details]
        [self.billing_details_container.nametowidget('{0}_container.{0}_input'.format(
            i)).pack(side=TOP, expand=True, fill=X) for i, j, k in self.customer_details]
        self.billing_details_container.pack(
            padx=20, pady=(10, 20), expand=True, fill=X)
        ...
        [i.pack(side=LEFT, expand=True, fill=BOTH, padx=10)
         for i in self.add_remove_widget_container]
        [self.add_remove_container.nametowidget('{0}_container.{0}_lb'.format(v[0])).pack(
            side=TOP, expand=True, fill=X, pady=(0, 5)) for i, v in enumerate(self.add_remove)]
        [self.add_remove_container.nametowidget('{0}_container.{0}_input'.format(v[0])).pack(
            side=TOP, expand=True, fill=X) for i, v in enumerate(self.add_remove)]
        ...
        self.receipt_view_container.pack(
        ...
        [v.pack(fill=BOTH, expand=True, side=LEFT, padx=10)
         for i, v in enumerate(self.function_btns)]
        ...
        self.product_lst_frm.pack(
            fill=BOTH, expand=True, side=TOP, pady=(20, 0))
        self.add_remove_frm.pack(
            fill=BOTH, expand=True, side=TOP, pady=(20, 0))
        self.purchased_lst_frm.pack(
            fill=BOTH, expand=True, side=TOP, pady=(20, 0))
        self.functions_frm.pack(fill=BOTH, expand=True,
                                side=BOTTOM, pady=(20, 0))

    def config_treeview(self):
        self.product_lst['columns'] = (
            'SN', 'PC', 'C', 'PN', 'U', 'UP', 'TR', 'AL')
        size = tuple(map(lambda i: int(i/1920*width),
                     (100, 70, 150, 200, 380, 120, 100, 100, 120)))
        text = ('SNO', 'Product Code', 'Category', 'Product Name',
                'Unit', 'Unit Price', 'Tax Rate', 'Amount Left',)
        [self.product_lst.column(i, anchor=W, minwidth=10, width=j, stretch=NO)
         for i, j in zip(('#0',)+self.product_lst['columns'], size)]
        [self.product_lst.heading(i, text=j, anchor=W)
         for i, j in zip(self.product_lst['columns'], text)]
        self.show_product_lst()

        self.purchased_lst['columns'] = (
            'SN', 'PC', 'PN', 'A', 'U', 'UP', 'TR', 'TT', 'TA')
        size = tuple(map(lambda i: int(i/1920*width),
                     (0, 70, 150, 535, 50, 120, 100, 100, 100, 120)))
        text = ('SNO', 'Product Code', 'Product Name', 'Qty', 'Unit',
                'Unit Price', 'Tax Rate', 'Tax Amt', 'Total Price')
        [self.purchased_lst.column(i, anchor=W, minwidth=10, width=j, stretch=NO)
         for i, j in zip(('#0',)+self.purchased_lst['columns'], size)]
        [self.purchased_lst.heading(i, text=j, anchor=W)
         for i, j in zip(self.purchased_lst['columns'], text)]

    def bind_event(self):
        self.product_lst.bind('<<TreeviewSelect>>',
                              self.product_lst_select_callback)
        self.quantity_input.bind('<FocusIn>', self.select_all)
        self.product_sno_input.bind('<FocusIn>', self.select_all)
        self.quantity_input.bind(
            '<FocusOut>', lambda e: e.widget.select_clear())
        self.product_sno_input.bind(
            '<FocusOut>', lambda e: e.widget.select_clear())

        self.product_lst.configure(
            yscrollcommand=self.product_lst_scrollbar.set)
        self.purchased_lst.configure(
            yscrollcommand=self.purchased_lst_scrollbar.set)

    def initialize_entry(self):
        self.bill_no_input.insert(0, 'RC210000001')
        self.bill_date_input.insert(
    ...
    def show_product_lst(self):
        ...
        for i, v in enumerate(categories):
            ...
            products = self.cursor.execute(
                'SELECT * FROM products WHERE category="{}"'.format(v[0])).fetchall()
            for v in products:
    ...
    def clear_add_remove_value(self, master, edit=False):
        ...
        [master.nametowidget('{0}_container.{0}_input'.format(i) if not edit else i+'_input').delete(0, 'end') if 'quantity' not in i else master.nametowidget(
            '{0}_container.{0}_input'.format(i) if not edit else i+'_input').set('') for i in self.purchased_data_order]
    ...
    def insert_add_remove_value(self, master, selection, include_category=True, insert_procuct_code=True, edit=False):
        ...
        if insert_procuct_code:
            self.product_code_input.insert(0, selection[1])
        self.product_name_input.insert(
            0, selection[3 if include_category else 2])
        ...
        self.tax_amount_input.insert(
            0, round(float(selection[6])*float(selection[5])/100, 2))
        self.total_price_input.insert(
            0, round(float(selection[5])+float(self.tax_amount_input.get()), 2))
    ...
    def product_lst_select_callback(self, e):
        selection = self.product_lst.item(
            self.product_lst.selection()[0])['values']
        if len(selection) == 8:
    ...
    def add_item_to_purchase(self):
        if all(self.add_remove_container.nametowidget('{0}_container.{0}_input'.format(i)).get() for i in self.purchased_data_order):
            self.purchased_lst.insert(parent='', index='end', values=[self.add_remove_container.nametowidget(
                '{0}_container.{0}_input'.format(i)).get() for i in self.purchased_data_order])
            self.update_summary()
            self.change_add_remove_state(NORMAL)
            [[self.add_remove_container.nametowidget('{0}_container.{0}_input'.format(
                i)).delete(0, 'end')] for i in self.purchased_data_order]
            self.change_add_remove_state(DISABLED)

    def update_summary(self):
        total, tax = list(map(
            lambda i: "{:.2f}".format(sum(i)) if i else "0.00",
            zip(*[
        ...
        self.grand_total_input.insert(
            0, round(float(self.total_input.get())+float(self.tax_input.get()), 2))
    ...
    def update_amount(self, P):
        ...
        try:
            ...
            self.tax_amount_input.insert(0, round(float(
                P)*float(self.tax_rate_input.get())*float(self.unit_price_input.get())/100, 2))
            self.total_price_input.insert(0, round(float(
                self.unit_price_input.get())*float(P)+float(self.tax_amount_input.get()), 2))
        except:
            pass
        self.change_add_remove_state(DISABLED)
        return True
    ...
    def delete_summary(self, total, tax):
        ...
        self.grand_total_input.delete(0, 'end')
        self.grand_total_input.insert(
            0, abs(round(float(self.total_input.get())+float(self.tax_input.get()), 2)))

    def delete_purchased(self):
        if self.purchased_lst.selection():
            selection = self.purchased_lst.item(
                self.purchased_lst.selection()[0])['values']
            self.purchased_lst.delete(self.purchased_lst.selection()[0])
            self.delete_summary(
                float(selection[3])*float(selection[5]), float(selection[7]))

    def find_product_by_code(self, P, master=None, edit=False):
        try:
            selection = self.cursor.execute(
                f'SELECT * FROM products WHERE product_code="{P}"').fetchone()
            if selection:
                self.insert_add_remove_value(
                    master, selection, insert_procuct_code=False, edit=edit)
        except:
            pass
        return True
    ...
    def find_product_by_sno(self, P, master=None, edit=False):
        try:
            selection = self.cursor.execute(
                f'SELECT * FROM products WHERE id={P}').fetchone()
            if selection:
                self.insert_add_remove_value(master, selection, edit=edit)
        except:
            pass
        return True
    ...
    def edit(self):
        if self.purchased_lst.selection():
            ...
            self.editwindow = EditWindow(
                self, selection_content[0], selection_id)
...
class DatabaseFrame(Frame):
    ...
    def define_widget(self):
        ...
        self.table_name_input_lb = ttk.Label(
            self.table_name_input_container, text='Table Name')
        self.table_name_input_input = ttk.Combobox(
            self.table_name_input_container)

        self.table_name_input_input['values'] = [i[0].replace('_', ' ').title(
        ) for i in self.cursor.execute("SELECT name FROM sqlite_master WHERE type='table';").fetchall()]
        self.table_name_input_input['state'] = 'readonly'
        self.table_name_input_input.set('Products')
        self.table_name_input_input.bind(
            '<<ComboboxSelected>>', self.table_name_change_callback)

        self.search_input_container = Frame(self.top_container)
        self.search_input_lb = ttk.Label(
            self.search_input_container, text="Search Item")
        self.search_input_input = ttk.Entry(self.search_input_container)

        self.database_table = ttk.Treeview(self.left_container)
        self.database_table_x_scrollbar = ttk.Scrollbar(
            self.left_container, command=self.database_table.xview, orient='horizontal')
        self.database_table_y_scrollbar = ttk.Scrollbar(
            self.left_container, command=self.database_table.yview, orient='vertical')
    ...
    def place_widget(self):
        ...
        self.table_name_input_container.grid(
            row=0, column=0, sticky=NSEW, padx=(0, 20))
        ...
        self.database_table.configure(
            yscrollcommand=self.database_table_y_scrollbar.set)
        self.database_table.configure(
            xscrollcommand=self.database_table_x_scrollbar.set)

    def config_treeview(self):
        table_name = self.table_name_input_input.get().replace(' ', '_').lower()
        column_names_raw = self.cursor.execute(
        ...
        [self.database_table.column(i, anchor=W, minwidth=j, width=j, stretch=NO) for i, j in zip(
            ('#0',)+self.database_table['columns'], column_width)]
        [self.database_table.heading(i, text=j, anchor=W) for i, j in zip(
            self.database_table['columns'], column_names)]
    ...
    def show_database(self, table_name):
        ...
        if table_name == 'products':
            categories = self.cursor.execute(
                'SELECT DISTINCT category FROM '+table_name).fetchall()
            for i, v in enumerate(categories):
                self.database_table.insert('', 'end', v[0], text=v[0])
                products = self.cursor.execute(
                    'SELECT * FROM {} WHERE category="{}"'.format(table_name, v[0])).fetchall()
                for v in products:
...
class EditWindow(Toplevel, BillingFrame):
    ...
    def define_widget(self):
        ...
        edit = [
            ('product_sno', 'SNO', 10, (self.register(lambda P: self.find_product_by_sno(
                P, self.widget_container, edit=True)), '%P'), NORMAL, 'xterm'),
            ('product_code', 'Product Code', 15, (self.master.register(lambda P: self.find_product_by_code(
                P, self.widget_container, edit=True)), '%P'), NORMAL, 'xterm'),
            ('product_name', 'Product Name', 40, lambda: True, DISABLED, 'arrow'),
            ('quantity', 'Qty', 5, (self.register(self.update_amount), '%P'),
             NORMAL, 'xterm', lambda: self.update_amount(self.quantity_input.get())),
        ...
        edit_lb = {
            i+'_lb': ttk.Label(self.widget_container, text=j, name=i+'_lb') for i, j, *_ in edit}
        edit_input = {i+'_input': (ttk.Entry if i != 'quantity' else ttk.Spinbox)(self.widget_container, **(dict(width=k, name=i+'_input', validate='key', validatecommand=l,
                                                                                                                 state=m, cursor=n, command=o[0] if o else None) | ({'from_': 1, 'to': 9999} if i == 'quantity' else {}))) for i, _, k, l, m, n, *o in edit}
        self.__dict__.update({**edit_lb, **edit_input})

        self.reset_btn = ttk.Button(self.widget_container, text='Reset', style='danger.Outline.TButton',
                                    width=10, command=lambda: self.insert_add_remove_value(self.widget_container, self.original_data))
        self.cancel_btn = ttk.Button(self.btn_container, text='Cancel',
                                     style='primary.Outline.TButton', width=10, command=self.destroy)
        self.save_btn = ttk.Button(self.btn_container, text='Save',
                                   style='primary.TButton', width=10, command=self.save_and_close)
    ...
    def place_widget(self):
        ...
        [self.widget_container.nametowidget(
            v+'_lb').grid(row=i, column=0, sticky=W, pady=(0, 15)) for i, v, in enumerate(order)]
        [self.widget_container.nametowidget(v+'_input').grid(row=i, column=1, padx=(
            20, 0), pady=(0, 15), sticky=W) for i, v, in enumerate(order)]
    ...
    def save_and_close(self):
        if all(self.widget_container.nametowidget(i+'_input').get() for i in self.purchased_data_order):
            ...
            self.main_window.purchased_lst.insert(parent='', index=int(self.treeview_id[1:]), values=[
                                                  self.widget_container.nametowidget(i+'_input').get() for i in self.main_window.purchased_data_order])
